...
def get_args():
    ...
    # three file .a3m .hhr .atab
    parser.add_argument(
        "-inputs",
        help="R|Input data in format A:B:C:D, with\n"
    ...
    if DEBUG:
        args = parser.parse_args(
            "-inputs PR:/home/huabei/project/github/RoseTTa-miRP/example/rna_pred/rna_binding_protein.RNA.a3m:/home/huabei/project/github/RoseTTa-miRP/example/rna_pred/rna_binding_protein.hhr:/home/huabei/project/github/RoseTTa-miRP/example/rna_pred/rna_binding_protein.atab -prefix /home/huabei/project/github/RoseTTa-miRP/example/rna_pred/models/model -model /home/huabei/project/github/RoseTTa-miRP/network/weights/RF2NA_apr23.pt -db /home/huabei/project/github/RoseTTa-miRP/pdb100_2021Mar03/pdb100_2021Mar03".split()
        )
    else:
        args = parser.parse_args()
    return args
...
class Predictor:
    ...
    def predict(self, inputs, out_prefix, ffdb, n_templ=4):
        """inputs: file1(a3m):file2(hhr):file3(atab)"""
        # pass 1, combined MSA
        # Ls: [protein length, dna length, rna length, protein length, rna length]
        Ls, msas, inss, types = [], [], [], []
        has_paired = False
        for i, seq_i in enumerate(inputs):
            ...
            if fseq_i[0] == "PR":
                ...
                if fseq_i[0] == "P":
                    ...
                    msa_i, ins_i = parse_fasta(
                        a3m_i, rna_alphabet=is_rna, dna_alphabet=is_dna
                    )

                _, L = msa_i.shape
                Ls.append(L)

            msa_i = torch.tensor(msa_i).long() # (N, Lp+Lr)
            ins_i = torch.tensor(ins_i).long() # (N, Lp+Lr) 全0
        ...
        if has_paired:
            if len(Ls) != 2 or len(msas) != 1:
                print(
                    "ERROR: Paired Protein/NA fastas can not be combined with other inputs!"
                )
                assert False
        else:
            for i in range(1, len(Ls)):
                msa_orig = merge_a3m_hetero(
                    msa_orig, {"msa": msas[i], "ins": inss[i]}, [sum(Ls[:i]), Ls[i]]
                )
        ...
        # 找到核苷酸的索引
        is_NA = util.is_nucleic(msa_orig[0]) # 数字大于21的是核苷酸
        # 初始化每个核苷酸O-P-O的坐标
        ...
        t1d = torch.nn.functional.one_hot(
            torch.full((n_templ, L), 20).long(), num_classes=NAATOKENS - 1
        ).float()  # all gaps '-'(20)
        # 分开是多余的
        t1d = torch.cat((t1d, torch.zeros((n_templ, L, 1)).float()), -1)

        maxtmpl = 1 # 最大模板数
        # 单独预测蛋白质的模板，在复合体预测中不起作用
        for i, seq_i in enumerate(inputs):
            fseq_i = seq_i.split(":")
            fseq_i[0] = fseq_i[0].upper()
            # 使得预测复合体也是用模板
            if fseq_i[0] in ["P"] and len(fseq_i) == 4:
                ...
                xyz_t_i, t1d_i, mask_t_i = read_templates(
                    Ls[i], ffdb, hhr_i, atab_i, n_templ=n_templ
                )
        ...
        alpha, _, alpha_mask, _ = self.xyz_converter.get_torsions(
            ...
            seq_tmp,
            mask_in=mask_t.reshape(-1, L, NTOTAL),
        )
    ...
    def _run_model(
        ...
        with torch.no_grad():
            seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(
                ...
                ins_orig,
                p_mask=0.0,
                params={"MAXLAT": MAXLAT, "MAXSEQ": MAXSEQ, "MAXCYCLE": MAX_CYCLE},
            )
            ...
            for i_cycle in range(MAX_CYCLE):
                ...
                _, all_crds = self.xyz_converter.compute_all_atom(
